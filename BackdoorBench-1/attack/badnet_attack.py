...
def main():
    ...
    '''这段代码是一个用于生成针对后门攻击的训练数据集的过程。它调用了一个名为`prepro_cls_DatasetBD`的函数，并传入了一系列参数：
    - `deepcopy(train_dataset_without_transform)`：这可能是原始的训练数据集，经过某些处理后得到的一个副本，`deepcopy`函数用于创建该副本。
    - `poison_idx=train_pidx`：这个参数可能是指示了哪些样本是有毒的（即包含后门的样本），`train_pidx`可能是一个索引列表。
    - `bd_image_pre_transform=train_bd_img_transform`：这是对后门图像进行预处理的转换函数。
    - `bd_label_pre_transform=bd_label_transform`：这是对后门标签进行预处理的转换函数。
    - `ori_image_transform_in_loading=train_img_transform`：这是在加载时对原始图像进行的转换。
    - `ori_label_transform_in_loading=train_label_transform`：这是在加载时对原始标签进行的转换。
    - `add_details_in_preprocess=True`：可能是一个标志，指示是否在预处理过程中添加一些额外的细节信息。
    总的来说，这段代码的目的是准备一个针对后门攻击的训练数据集，其中包含一些有毒的样本，并对这些样本进行了必要的预处理，以便于后续的训练过程。'''
