...
def orders(request):
    if request.method == 'GET':
        ...
        # 创建分页器对象
        paginator_file_one = Paginator(grouped_data["文件一"], page_size)
        paginator_file_two = Paginator(grouped_data["文件二"], page_size)
        paginator_file_tre = Paginator(grouped_data["文件三"], page_size)
        paginator_file_for = Paginator(grouped_data["文件四"], page_size)
        # 获取当前页码，默认为1
