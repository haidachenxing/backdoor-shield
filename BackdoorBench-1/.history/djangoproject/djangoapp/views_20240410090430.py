...
# 通常情况下，建议您在命名视图函数时使用有意义的命名，以便于识别它们的功能。并且建议在编写文档或者注释时注明每个视图函数负责渲染哪些页面或者实现了哪些功能。
# 这样做可以提高代码的可读性和可维护性，同时也有助于未来对代码的修改和扩展。

# Django的开发流程就是找到urls里对应的路径，再找到对应的函数，函数之间可以通过redirect串起来，可能多个函数一起渲染了一个页面。所以学会使用redirect和render是至关重要的！！！
...
def execute_defense(request):
    if request.method == 'POST':
        ...
        # 如果这里用render(reuest, 'analyse.html'),无法渲染analyse.html页面，因为analyse.html页面中有一些需要后端处理(在analyse函数)的数据，所以需要用redirect()函数来重定向到analyse函数中
        # 如果render到index.html页面，就不需要重定向，因为index.html页面中没有需要后端处理的数据
    else:
        return render(request, 'upload.html')
...
def analyse(request):
    ...
    return render(request, 'analyse.html', {'defense_algorithm': defense_algorithm, 'file_name': file_name, 'file_size': file_size, 'log_file_content': log_file_content, 'message': message, 'analyse_algorithm': analyse_algorithm})

def execute_analyse(request):
    if request.method == 'POST':
        analyse_algorithm = request.POST.get('analyse_algorithm')
        request.session['analyse_algorithm'] = analyse_algorithm
...
def orders(request):
    if request.method == 'GET':
        # 假设你有一个从数据库或其他来源获取的数据字典
        data_dict = {
            (1,): {"num": 1, "name": "file_1", "type": "数据集防御", "data": "2024-3-20", "state": "已完成",
                   "scale": "102.87675952911377 MB"},
            (2,): {"num": 2, "name": "file_2", "type": "模型防御1", "data": "2024-3-20", "state": "已完成",
                   "scale": "105.42975372911376 MB"},
            (3,): {"num": 3, "name": "file_2", "type": "模型防御2", "data": "2024-3-20", "state": "未完成",
                   "scale": "105.42975372911376 MB"},
            (4,): {"num": 4, "name": "file_2", "type": "模型防御3", "data": "2024-3-20", "state": "已完成",
                   "scale": "105.42975372911376 MB"},
            (5,): {"num": 5, "name": "file_2", "type": "模型防御4", "data": "2024-3-20", "state": "已取消",
                   "scale": "105.42975372911376 MB"},
        }  # 示例数据字典

        grouped_data = {
            "file_1": [item[1] for item in data_dict.items() if item[1]['state'] == "已完成"],
            "file_2": [item[1] for item in data_dict.items() if item[1]['state'] == "未完成"],
            "file_3": [item[1] for item in data_dict.items() if item[1]['state'] == "已取消"],
            "file_4": [item[1] for item in data_dict.items()],
        }
        ...
        # 创建分页器对象
        paginator_file_one = Paginator(grouped_data["file_1"], page_size)
        paginator_file_two = Paginator(grouped_data["file_2"], page_size)
        paginator_file_tre = Paginator(grouped_data["file_3"], page_size)
        paginator_file_for = Paginator(grouped_data["file_4"], page_size)
        # 获取当前页码，默认为1
        page_number_file_one = request.GET.get('page_file_one', 1)
        page_number_file_two = request.GET.get('page_file_two', 1)
        page_number_file_tre = request.GET.get('page_file_tre', 1)
        page_number_file_for = request.GET.get('page_file_for', 1)
        # 获取对应页码的对象
        page_obj_file_one = paginator_file_one.get_page(page_number_file_one)
        page_obj_file_two = paginator_file_two.get_page(page_number_file_two)
        page_obj_file_tre = paginator_file_tre.get_page(page_number_file_tre)
        page_obj_file_for = paginator_file_for.get_page(page_number_file_for)
        # 将分页对象传递给模板
        return render(request, 'orders.html', {
            'page_obj_file_one': page_obj_file_one,
            'page_obj_file_two': page_obj_file_two,
            'page_obj_file_tre': page_obj_file_tre,
            'page_obj_file_for': page_obj_file_for,
        })
    return render(request, 'orders.html')
def help(request):
