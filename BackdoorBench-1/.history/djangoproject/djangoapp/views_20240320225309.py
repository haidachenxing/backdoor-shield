...
def download_log(request):
    user_email = request.session.get('user_email')
    defense_algorithm = request.session.get('defense_algorithm')
    # log_file_path = 'D:/study/work/backdoorbench/BackdoorBench-1/djangoproject/record' + '/' + user_email + '/saved' + '/' + defense_algorithm + '/log.txt'
    logs_folder_path = 'D:/study/work/backdoorbench/BackdoorBench-1/djangoproject/record' + '/' + user_email + '/' + defense_algorithm
    # 检查日志文件是否存在
    if os.path.exists(log_file_path):
        ...
        with open(log_file_path, 'rb') as log_file:
            ...
            # 设置响应的文件名
            response['Content-Disposition'] = 'attachment; filename="logfile.log"'
            return response
    else:
        # 如果日志文件不存在，则返回 404 错误
