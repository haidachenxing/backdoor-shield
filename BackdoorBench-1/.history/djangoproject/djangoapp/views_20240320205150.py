...
def execute_analyse(request):
    if request.method == 'POST':
        ...
        if analyse_algorithm = "1": # T-SNE
            ...
            # T-SNE
            # ++++++++++++++++++++++++++++++++++
            tsne_path = "D:/study/work/backdoorbench/BackdoorBench-1/visualization/tsne.py"
            tsne_command = [
                "python", tsne_path,
                "--result_file", user_email
            ]
            print("tsne命令路径：" + " ".join(tsne_command))
            subprocess.run(tsne_command, check=True)
