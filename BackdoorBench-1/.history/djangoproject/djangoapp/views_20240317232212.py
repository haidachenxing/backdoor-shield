...
from django.conf import settings  
from djangoapp import my_models
import sqlite3
...
def user_login(request):
    ...
    if request.method == 'POST':
        ...
        found_email = my_models.User.objects.filter(email=email).first()
        print("获取到信息")
        print(email)
        print(password)
        print(found_email)
...
def user_signup(request):
    ...
    if request.method == 'POST':
        ...
        if found_email:
            ...
            new_user = my_models.User(username=username, email=email, password=password)
            new_user.save()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if uploaded_file.name.endswith('.pt'):
                # 保存上传的文件到服务器固定路径
                file_path = os.path.join('upload_file', uploaded_file.name)
                # 确保 upload_file 文件夹存在，如果不存在则创建它
...
def ajax_path_dealer(request):
    if request.method == 'POST' and request.is_ajax():
        ...
        # 构建绝对路径
        denfense_path = os.path.join(current_directory, "defense\\ac\\ac.py")
        yaml_path = os.path.join(current_directory, "config\\defense\\ac\\cifar10.yaml")
        input_path = os.path.join(current_directory, "upload_file\\attack_result.pt")
        output_path = os.path.join(current_directory, "upload_file\\result.pt")
