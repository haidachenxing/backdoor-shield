...
def get_args():
    ...
    parser.add_argument("--device", type=str, help="cuda, cpu") # cuda = 0
    parser.add_argument(
        "--yaml_path",
        type=str,
        default="D:/study/work/backdoorbench/BackdoorBench-1/config/visualization/default.yaml", # 原来是"./config/visualization/default.yaml"
        help="the path of yaml",
    ) 
    parser.add_argument("--seed", type=str, help="random seed") # 0
    parser.add_argument("--model", type=str, help="preactresnet18") # preactresnet18

    # data parameters
    parser.add_argument("--dataset_path", type=str) # "./data"
    parser.add_argument(
        "--dataset", type=str, help="mnist, cifar10, cifar100, gtsrb, celeba, tiny"
    ) # "cifar10"
    parser.add_argument("--num_classes", type=int) # 根据不同的dataset来定
    parser.add_argument("--input_height", type=int) # 根据不同的dataset来定
    parser.add_argument("--input_width", type=int) # 根据不同的dataset来定
    parser.add_argument("--input_channel", type=int) # 根据不同的dataset来定
    parser.add_argument("--batch_size", type=int) # 128
    parser.add_argument("--n_sub", default=5000, type=int) # 5000
    parser.add_argument("--c_sub", default=10, type=int) # 10
    parser.add_argument("--num_workers", default=0, type=float) # 0 for windows， 4 for linux
    parser.add_argument("--class_names", type=list,
                        help="names for each class") # 根据不同的dataset来定

    # BD parameters
    parser.add_argument("--pratio", type=float) # 0.1
    parser.add_argument("--attack_target", type=int) # 0
    ...
    parser.add_argument("--checkpoint_load", default=None, type=str) # 不用管
    parser.add_argument("--checkpoint_save", default=None, type=str) # 不用管
