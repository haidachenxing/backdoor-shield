...
# note: 这个xy_iter类是一个自定义的PyTorch数据集类，它继承了torch.utils.data.Dataset。这个类的主要目的是包装输入数据x和对应的目标y，并提供一种迭代这些数据的方式
...
class prepro_cls_DatasetBD(torch.utils.data.dataset.Dataset): # note: a class for preprocessing datasets containing backdoor attacks
                                                              # note: prepro: preprocessing; cls: class; DatasetBD: DatasetBackDoor即被后门攻击的数据集
