...
# note: 这段代码实现了一个名为 MixMatch 的半监督学习算法的训练过程。
# note: MixMatch 是一种结合了混合样本（Mixup）和一致性正则化（Consistency Regularization）的方法，它同时对有标签和无标签的数据进行训练，以提高模型的泛化能力。
# note: 总的来说，这段代码实现了 MixMatch 算法的训练过程，包括数据加载、无标签数据预测、Mixup 操作、模型预测、损失计算和优化，以及训练和输出的记录。
def mixmatch_train(
