...
from utils.bd_dataset import prepro_cls_DatasetBD # note: a class for preprocessing datasets containing backdoor attacks
...
# note: train(args, result)函数里套用了函数train_step(args, train_loader, model_ascent, optimizer, criterion, epoch)
def train(args, result):
    ...
    optimizer = torch.optim.SGD(model_ascent.parameters(),
                                ...
                                nesterov=True) # note: SGD优化，全称为随机梯度下降（Stochastic Gradient Descent），是深度学习中的一种函数优化方法。

    # define loss functions
    if args.device == 'cuda':
        criterion = torch.nn.CrossEntropyLoss().cuda() # note: criterion是损失函数
    else:
    ...
    x = result['bd_train']['x'] # note: 这里x和y代表什么？
    y = result['bd_train']['y']
    data_set = list(zip(x,y))
    poisoned_data = prepro_cls_DatasetBD( # note: 为啥dbd里面还能返回干净的样本？462行左右
...
def train_step_finetuing(args, train_loader, model_ascent, optimizer, criterion, epoch): # note: dbd和abl都有微调，之后看完ft.py会知道fine turn的含义
